...
juce::Image AboutComponent::createIcon (int size)
{
    ...
    for (int row = 0; row < numRows; ++row)
    {
        ...
        for (int bar = 0; bar < numBars; ++bar)
        {
            ...
            juce::Colour col = cyan.interpolatedWith (pink, t)
                                   .withAlpha (0.20f + 0.80f * rowAge);
            g.setColour (col);
...
void AboutComponent::paint (juce::Graphics& g)
{
    ...
    g.drawText ("Version " + juce::String (AppVersion::string),
                textX, y, textW, 18, juce::Justification::centredLeft);
    y += 22;

    g.drawText ("CUDA Runtime  " + cudaRuntimeStr,
                textX, y, textW, 18, juce::Justification::centredLeft);
    y += 20;

    g.drawText ("CUDA Driver   " + cudaDriverStr,
                textX, y, textW, 18, juce::Justification::centredLeft);
    y += 28;

    g.setColour (juce::Colour (0xff5555aa));
    g.drawText ("Real-time 3D spectral waterfall visualizer.",
                textX, y, textW, 18, juce::Justification::centredLeft);
    y += 20;
    g.drawText ("CUDA + OpenGL \u2022 JUCE 8 \u2022 NVIDIA Ada (SM 8.9)",
                textX, y, textW, 18, juce::Justification::centredLeft);
